...
`packer_compiled.lua` is generated by `:PackerSync` and is gitignored
in `~/.gitignore` — its bytecode re-serializes with different table
iteration order on every regen, so tracking it produced constant churn.
Run `:PackerSync` after cloning to materialize it locally.
